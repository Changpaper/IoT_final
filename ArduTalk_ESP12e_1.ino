//   ArduTalk
#define DefaultIoTtalkServerIP "140.113.199.249"
...
void handleRoot(int retry)
{
    ...
    if (retry)
        temp += "<font color=\"#FF0000\">Please fill all fields.</font>";
...
void saveInfoAndConnectToWiFi()
{
    ...
    if (server.arg(0) != "" && server.arg(2) != "")
    { // arg[0]-> SSID, arg[1]-> password (both string)
...
void loop()
{
    ...
    if (millis() - cycleTimestamp > 200)
    {
        ...
        if (result != "___NULL_DATA___")
        {
            ...
            if (result.toInt() >= 0 && result.toInt() <= 255)
